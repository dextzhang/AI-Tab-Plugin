...
### v2.0.0 Text Sync Stable Milestone

- User confirmed the text synchronization flow is now highly complete and basically free of major bugs.
- Promoted the project version from `1.6.2` to `2.0.0`.
- This milestone represents the first stable text-sync release after the popup, background worker, persistent logs, site adapters, Kimi fresh-chat handling, and Qwen/Kimi single-write fixes.
- Text sending is now considered the stable core capability.
- Remaining future work should focus on image generation reliability, additional site-specific edge cases, and product polish rather than the main text broadcast path.
